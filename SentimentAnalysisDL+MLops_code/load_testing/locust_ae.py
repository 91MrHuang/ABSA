...
ae_reader = CSVReader('test_data/ae.csv')

class AEUser(HttpUser):
    ...
    @tag('load testing for Aspect level Extraction')
    @task(1)
    def do_ae(self):
        test_sentences = json.loads(next(ae_reader)[0].replace('\'', '"'))
    ...
    def _(environment, **kw):
        fh = logging.FileHandler('ae_load_test.log')
        logger = logging.getLogger('ae_load_test')
